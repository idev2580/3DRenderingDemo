...
function buildAnimationTimeline(animations, minimumDuration = 1) {
...
function createCameraTrajectoryEditor({
  ...
  const timeline = buildAnimationTimeline(animations, minimumDuration);
  const state = {
    currentTime: 0,
  ...
  function applyTimelineTime() {
    if (mixer) {
      const mixerTime = Math.min(state.currentTime, Math.max(timeline.duration - 0.0001, 0));
  ...
  function update(delta) {
    if (state.playbackEnabled) {
      state.currentTime = Math.min(state.currentTime + delta, timeline.duration);
...
const cameraTrajectoryEditor = createCameraTrajectoryEditor({
  ...
  mixer,
  onTimeChange: (time) => {
    applySplatTimelineAt(time);
  },
  minimumDuration: splatTimelineEndTime,
...
const splatTimelineDuration =
  Math.max(sceneTimeline.duration, thresholdTime + switchAnimationLength, splatFadeEndTime) + 0.001;
const splatTimelineState = {
  t: bgSplatTimeOffset,
  bgAlpha: 1,
  bg2Alpha: 0,
  texAlpha: 1,
};
const splatTimelineMixer = new THREE.AnimationMixer(splatTimelineState);
const splatTimelineClip = new THREE.AnimationClip("SplatTimeline", splatTimelineDuration, [
  new THREE.NumberKeyframeTrack(
    ".t",
    [0, thresholdTime, thresholdTime + switchAnimationLength, splatTimelineDuration],
    [
      bgSplatTimeOffset,
      bgSplatTimeOffset,
      bgSplatTimeOffset + switchAnimationLength * 2,
      bgSplatTimeOffset + switchAnimationLength * 2,
    ]
  ),
  new THREE.NumberKeyframeTrack(
    ".bgAlpha",
    [0, splatFadeStartTime, splatFadeEndTime, splatTimelineDuration],
    [1, 1, 0, 0]
  ),
  new THREE.NumberKeyframeTrack(
    ".bg2Alpha",
...
const splatTimelineAction = splatTimelineMixer.clipAction(splatTimelineClip);

splatTimelineAction.setLoop(THREE.LoopOnce, 1);
splatTimelineAction.clampWhenFinished = true;
splatTimelineAction.play();

function syncSplatTimelineState() {
  bgSplatAnimateT.value = splatTimelineState.t;
  bgSplatAlpha.value = splatTimelineState.bgAlpha;
  bg2SplatAlpha.value = splatTimelineState.bg2Alpha;
  texSplatAlpha.value = splatTimelineState.texAlpha;
  if (transitionObject) {
    setObjectOpacity(transitionObject, splatTimelineState.bg2Alpha);
  }
...
applySplatTimelineAt = (time) => {
  splatTimelineMixer.setTime(time);
  syncSplatTimelineState();
};

applySplatTimelineAt(cameraTrajectoryEditor.getCurrentTime());
